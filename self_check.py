...
@check("3-5e. 리포트 주력상품 핵심어 추출 (유형어 사전)")
def _():
    from report_generator import _product_keyword

    # 실제 수집된 상품명들. 핵심어는 '이 제품이 무엇인지'를 말해야 한다.
    #   (브랜드명 + 이 핵심어로 네이버를 검색해 '실시간 시장 반응'을 뽑기 때문에,
    #    핵심어가 빗나가면 숫자는 맞아도 엉뚱한 라인을 재는 꼴이 된다)
    cases = [
        ("앙또미뇽",
         "앙또미뇽 아기 바디워시 신생아 유아 키즈 바스앤샴푸 올인원 대용량 약산성 500ml, 2개",
         "바스앤샴푸"),      # 예전엔 '올인원 약산성' — 제품 정체가 안 드러났다
        ("누엔쁘",
         "누엔쁘 맘조리 산모 산후 순환크림 임산부 화장품 출산선물",
         "순환크림"),        # 예전엔 '화장품 출산선물'
        ("닥터바이오 더마 코스메틱",
         "닥터바이오 진정 쿨링 수딩수분 로션 500g[신생아 아기 유아 페이스&바디]",
         "로션"),            # 예전엔 '페이스 바디'
        ("베베가닉",
         "베베가닉 신생아 아기 유아 바스앤샴푸 바디워시 280g 1개",
         "바스앤샴푸"),
    ]
    for brand, flagship, expect in cases:
        got = _product_keyword(brand, flagship)
        assert got == expect, f"{brand}: '{got}' (기대: '{expect}')"

    # 브랜드명·용량·수량·타겟어는 핵심어에 절대 섞이면 안 된다
    kw = _product_keyword("앙또미뇽", cases[0][1])
    for bad in ("앙또미뇽", "500", "ml", "신생아", "아기", "유아", "키즈", "대용량"):
        assert bad not in kw, f"핵심어에 '{bad}'가 섞임: {kw}"

    # 사전에 없는 유형이면 옛 방식(뒤 2단어)으로 폴백해야 한다 — 빈손이 되면 안 됨
    assert _product_keyword("테스트", "테스트 신생아 아기 무명제품 특수용품") != ""
    # 주력상품이 없으면 빈 문자열 (호출부가 브랜드 전반만 검색)
    assert _product_keyword("테스트", "") == ""
