...
def _market_signals(brand: str, category: str, flagship: str = "") -> dict:
    ...
    if kw and kw not in brand:
        ...
        base["posts"] = (_naver_blog_bodies(f"{brand} {kw}", 8)
...
아래 '{ctx['brand']}' 브랜드의 실제 제품 정보 + 주력상품을 검색해 읽은 실제 후기 본문
(네이버 블로그·카페)을 근거로 기획 콘텐츠를 만든다.
특히 후기에서 '구매자가 긍정적으로 느낀 점'을 찾아 그걸 이 브랜드가 밀 강점으로 발전시킨다.
...
- market_keywords는 ⭐창작이 아니라 '원문 인용'이다⭐. 그리고 ⭐긍정적인 의견만⭐ 뽑는다.
  위 후기 본문에서 구매자가 '긍정적으로 느낀 점'(만족한 점·좋았던 점·
  사길 잘했다고 느낀 이유·재구매하는 이유)이 드러난 구절을
  **토씨 하나 바꾸지 말고 그대로 오려서** 넣어라.
  · ⭐불만·걱정·아쉬웠던 점·부정적 표현은 넣지 마라⭐ — 이 칸은 '밀 만한 강점'을 찾는 자리다.
  · 요약·다듬기·맞춤법 교정 금지. 본문에 있는 글자 그대로.
    (본문에 없는 문장은 자동 폐기되어 아무 소용이 없다)
  · 한 구절 6~22자. 문장이 길면 긍정 체감이 드러난 부분만 잘라 인용하라.
  · 금지: 제품 유형명·카테고리명·브랜드명만 있는 구절
         (예: '바스앤샴푸', '아기바디워시', '육아용품') — 당연한 말이라 쓸모없다.
  · 좋은 인용: '눈에 거품이 들어가도 아프지 않은', '흡수가 빨라서 끈적임이 없어요',
               '아기가 자꾸 얼굴을 부비더라고요'
  · 긍정 구절이 부족하면 억지로 채우지 말고 개수를 줄여라.
- appeal_points는 위 긍정 인용(market_keywords)들을 '조합·분석해 광고 문구로 발전시킨 것'이다.
  순서를 지켜라:
  ① 구매자가 긍정적으로 한 말(market_keywords)들을 먼저 모은다
  ② 여러 후기에 공통으로 흐르는 '사는 이유·강점'을 뽑아 짧은 소구 키워드로 다듬는다
     (여러 후기가 같은 얘기를 반복하면 그게 곧 이 브랜드의 핵심 강점이다)
  예) 인용 '눈에 거품이 들어가도 아프지 않은' → 소구 '눈 안 맵는 순한 거품'
