...
{{
  ...
  "appeal_points": ["위 market_keywords(구매자가 실제로 한 말)를 광고 문구로 발전시킨 소구 키워드 5개 (# 없이)"],
...
- appeal_points는 위 market_keywords를 '광고 문구로 발전시킨 것'이다. 순서를 지켜라:
  ① 구매자가 실제로 한 말(market_keywords)을 먼저 정한다
  ② 그 말이 왜 사는 이유가 되는지 뽑아 짧은 소구 키워드로 다듬는다
  예) 인용 '눈에 거품이 들어가도 아프지 않은' → 소구 '눈 안 맵는 거품'
      인용 '하루에 두번이상은 물로'         → 소구 '하루 두 번도 부담 없이'
  · 후기와 무관한 일반 문구(‘엄마가 고른 브랜드’ 같은 말)는 쓰지 마라.
  · 이건 창작 카피라 표현은 자유지만, 출발점은 반드시 위 인용이어야 한다.
